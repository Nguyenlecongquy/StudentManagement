...
const studentModel = {
   ...
   addHSIntoDatabaseReturnHS: async (id, fullName, sex, birthDay, address, idClass) => {
      try {
         const result = await db.any(`insert into hoc_sinh(ma_hs, ten_hs, ma_lop, ngay_sinh_hs, gioi_tinh_hs, dia_chi_hs) 
   ...
   updateHSIntoDatabase: async (id, fullName, sex, birthDay, address, idClass) => {
      try {
         const result = await db.any(`update hoc_sinh 
                                 set  ten_hs=$1, ma_lop=$2, ngay_sinh_hs=$3, gioi_tinh_hs=$4, dia_chi_hs=$5
                                 where ma_hs=$6 returning *`, [fullName, idClass, birthDay, sex, address, id]);
         return result;
      } catch (error) {
         return false;
      }
