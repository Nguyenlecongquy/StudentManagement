...
class InforC {
   ...
   async search(req,res){
      ...
      const result = await inforM.findInfors(infor.email,infor.category);
   ...
   async modify(req, res) {
      ...
      const result = await inforM.updateInfors(infor.fullName,infor.email,infor.password,infor.category);
      if(result!= [] && result!= false ) response.infors = result;
      else response.status =false;
      res.status(200).json(response);
   }
